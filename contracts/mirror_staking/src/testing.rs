...
fn register_migration() {
    ...
    let res = handle(&mut deps, env, msg);
    match res {
        Err(StdError::GenericErr { msg, .. }) => assert_eq!(msg, "no pool data stored"),
        _ => panic!("DO NOT ENTER HERE"),
    }
    ...
    .unwrap();
    let pool_info_res: PoolInfoResponse = from_binary(&res).unwrap();
    assert_eq!(
        pool_info_res,
        PoolInfoResponse {
            asset_token: HumanAddr::from("asset0001"),
...
fn migrate_bonding() {
    ...
    // migrate bonding
    let msg = HandleMsg::MigrateBonding {
        asset_token: HumanAddr::from("asset0000"),
    };
    let env = mock_env("addr0000", &[]);
    let _res = handle(&mut deps, env, msg).unwrap();
    ...
    .unwrap();
    let reward_info_res: RewardInfoResponse = from_binary(&res).unwrap();
    assert_eq!(
        reward_info_res,
        RewardInfoResponse {
            staker: HumanAddr::from("addr0000"),
