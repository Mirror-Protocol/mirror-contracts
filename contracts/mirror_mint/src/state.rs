...
pub fn create_position<'a, S: Storage>(
    ...
    position_indexer.save(&idx.to_be_bytes(), &true)?;
...
pub fn remove_position<'a, S: Storage>(
    ...
    position_indexer.remove(&idx.to_be_bytes());
}
...
pub fn read_positions<'a, S: ReadonlyStorage>(
    ...
    position_indexer
        ...
        .map(|item| {
            let (k, _) = item?;
            let mut idx: [u8; 8] = Default::default();
            idx.copy_from_slice(&k[0..8]);
            read_position(storage, u64::from_be_bytes(idx))
        })
        .collect()
}
...
fn calc_range_start(start_after: Option<u64>) -> Option<Vec<u8>> {
    start_after.map(|idx| {
        let mut v = Vec::from(idx.to_be_bytes());
        v.push(1);
        v
    })
}
