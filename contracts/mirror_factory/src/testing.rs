...
use terraswap::{AssetInfo, InitHook, TokenInitMsg, TokenMigrationResponse};
...
fn test_migration() {
    ...
    let msg = HandleMsg::MigrationTokenCreationHook {};
    let env = mock_env("asset0001", &[]);
    let res = handle(&mut deps, env, msg).unwrap_err();
    match res {
        StdError::GenericErr { msg, .. } => assert_eq!(msg, "No migration process in progress"),
        _ => panic!("DO NOT ENTER HERE"),
    }
    ...
    let msg = HandleMsg::MigrateAsset {
        ...
        conversion_rate: Decimal::from_ratio(2u128, 1u128),
    };
    let env = mock_env("addr0000", &[]);
    ...
    assert_eq!(
        ...
        vec![CosmosMsg::Wasm(WasmMsg::Instantiate {
            ...
            msg: to_binary(&TokenInitMsg {
                ...
                init_hook: Some(InitHook {
                    contract_addr: HumanAddr::from(MOCK_CONTRACT_ADDR),
                    msg: to_binary(&HandleMsg::MigrationTokenCreationHook {}).unwrap(),
                }),
